...
class Unzipper {
  ...
  /** CelluleDev - Mise à jour bug detection zip tgz
  ...
  public static function extractGzipFile($archive, $destination)
  {
    ...
    if (file_exists($destination . '/' . $filename)) {
      ...
      // If we had a tgz file, let's extract that tar file.
      if (pathinfo($destination . '/' . $filename, PATHINFO_EXTENSION) == 'tgz') {
        $phar = new PharData($destination . '/' . $filename);
        if ($phar->extractTo($destination)) {
          $GLOBALS['status'] = array('success' => 'Extracted tgz archive successfully.');
          // Delete .tar.
          unlink($destination . '/' . $filename);
        }
      }
